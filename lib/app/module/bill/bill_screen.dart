...
class _SubBillScreenState extends State<SubBillScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final aramexProvider = Provider.of<AramexProvider>(context);
    DiscountVolume? discountVolume;
    var cartList = cartModel.items.where((element) => element.isPaidAdd == false).toList();
    ...
    try{
      ...
      if(newDeliveryPriceModel.isLoading || taxModel.isLoading){
        ...
        if (widget.shippingType == allTranslations.text("aramex")) {
          ...
          if (cartModel.totalPrice.round() <= double.parse(
              newDeliveryPriceModel.newDeliveryPrice.minimum.toString())) {
            setState(() {
              deliveryPrice = double.parse(
                  newDeliveryPriceModel.newDeliveryPrice.fixedDeliveryRate
                      .toString());
            });
          }
          else {
        ...
        if (taxModel.items.isNotEmpty) {
          tax = ((double.parse(cartModel.totalPrice.toString()) + deliveryPrice)
              * (double.parse(taxModel.taxPrice.price.toString()) / 100));
          setState(() {
            amountToBePaid = double.parse(cartModel.totalPrice.toString()) + deliveryPrice +
    ...
    Future sendRequest() async {
      ...
      if (cartModel.isLoaded) {
        ...
        if (requestModel.isLoaded) {
          if(widget.shippingType == allTranslations.text("aramex")){
            try {
              await aramexProvider.createShipment(widget.shipmentData).then((value) {
    ...
    Widget calculationDiscount(BuildContext context) {
      ...
      if(cartModel.items.isNotEmpty){
        /// discount of balance
        if (discountVolume != null && discountVolume.valueType == "price" &&
            discountVolume.type == "balance" &&
            chosenPaymentNumber == walletNumber ) {
          child = Row(
            mainAxisAlignment: MainAxisAlignment.start,
        ...
        else if (discountVolume != null && discountVolume.valueType == "percentage" &&
            discountVolume.type == "balance" &&
            chosenPaymentNumber == walletNumber ) {
          child = Text(
